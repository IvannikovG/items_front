...
* [`src/cljs/IvannikovG/items_exchange_front/`](src/cljs/IvannikovG/items_exchange_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/IvannikovG/items_exchange_front/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `items_exchange_front` project root directory
...
The `debug?` variable in [`config.cljs`](src/IvannikovG/items_exchange_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns IvannikovG.items-exchange-front.example
  (:require [IvannikovG.items-exchange-front.config :as config])
